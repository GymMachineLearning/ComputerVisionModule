...
def model_load(config):
    ...
    if torch.cuda.is_available():
        model = model.cuda()
        state_dict = torch.load(config.OUTPUT_DIR)  # load model weights for GPU
    else:
        state_dict = torch.load(config.OUTPUT_DIR, map_location=torch.device('cpu'))  # load model weights for CPU
    
    from collections import OrderedDict
    new_state_dict = OrderedDict()
    
    for k, v in state_dict.items():
        name = k  # możesz również zaktualizować, jeśli nazwy kluczy się różnią (np. usunąć 'module.' itp.)
        new_state_dict[name] = v
    
    model.load_state_dict(new_state_dict)
    model.eval()  # ustawienie modelu w trybie ewaluacji
    
    # print('HRNet network successfully loaded')  # opcjonalne logi
